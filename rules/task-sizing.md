...
```
/investigate → /{framework}-04-fix → /ship
```
- Skip explore, plan
...
```
/explore → /{framework}-01-plan → 02-implement → /simplify → 03-review → 04-fix → /qa → /tech-debt → /ship
```
- Standard pipeline
...
```
/explore → /plan-ceo-review → /{framework}-01-plan → 02-implement (--team) → 03-review (--team) → 04-fix → /qa → /tech-debt → /ship
```
