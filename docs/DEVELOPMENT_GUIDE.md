...
> AI-Native 개발 파이프라인 — 56개 스킬, 16개 에이전트, 자동화 훅 기반
...
────────── ──────────── ──────────────────────────────── ──────────── ────── ──────
/office    /generate    /explore → /{fw}-01-plan         /qa      /ship  /retro
...
| **M** | 3-10 파일 | `/explore` → `01-plan` → `02-implement` → `/simplify` → `03-review` → `04-fix` → `/qa` → `/tech-debt` → `/ship` |
...
| `/plan-eng-review` | 엔지니어링 아키텍처 리뷰 | 기술 설계 검증할 때 |
| `/plan-design-review` | 디자이너 관점 계획 리뷰 (0-10 스코어링) | UI/UX 계획 검토할 때 |
| `/office-hours` | 아이디어 정제 | 코드 전, 니즈 파악할 때 |
| `/batch` | 격리된 work tree에서 병렬 실행 | 마이그레이션, 일괄 리팩토링 |
| `/simplify` | 코드 단순화 + 품질 체크 | 복잡도 감소, 중복 제거 |
| `/qa` | 영향도 분석 + 정적 검증 + 테스트 + 브라우저 검증 + 자동 수정 | 구현 후 전체 검증 |
| `/qa-only` | 리포트 전용 QA (수정 없음) | 버그 리포트만 필요할 때 |
| `/review` | 프리랜딩 PR 코드 리뷰 (fix-first) | 브랜치 변경사항 리뷰할 때 |
...
| `/design-consultation` | 디자인 시스템 구축 (DESIGN.md) | 새 프로젝트 디자인 잡을 때 |
| `/design-review` | 라이브 사이트 디자인 감사 + 수정 | UI 품질 점검할 때 |
| `/document-release` | 배포 후 문서 동기화 | ship 후 문서 업데이트 |
| `/benchmark` | 성능 회귀 탐지 | 성능 측정할 때 |
| `/health` | 코드 품질 대시보드 (0-10 점수) | 전체 품질 체크할 때 |
| `/learn` | 프로젝트 학습 관리 | 세션 간 학습 조회/정리 |
| `/context-save` | 작업 컨텍스트 저장 | 세션 중단 시 |
| `/context-restore` | 저장된 컨텍스트 복원 | 이전 작업 재개할 때 |
| `/make-pdf` | 마크다운 → PDF 변환 | 문서 출력할 때 |
...
| Angular 새 기능 | `/explore` → `/angular-01-plan` → `02` → `/simplify` → `03` → `04` → `05` → `/qa` → `/tech-debt` → `/ship` |
| React 새 기능 | `/explore` → `/react-01-plan` → `02` → `/simplify` → `03` → `04` → `05` → `/qa` → `/tech-debt` → `/ship` |
| Kotlin 새 기능 | `/explore` → `/kotlin-01-plan` → `02` → `/simplify` → `03` → `04` → `05` → `/qa` → `/tech-debt` → `/ship` |
| Swift 새 기능 | `/explore` → `/swift-01-plan` → `02` → `/simplify` → `03` → `04` → `05` → `/qa` → `/tech-debt` → `/ship` |
| Node.js 새 기능 | `/explore` → `/nodejs-01-plan` → `02` → `/simplify` → `03` → `04` → `05` → `/qa` → `/tech-debt` → `/ship` |
| TypeScript 유틸리티 | `/ts-01-plan` → `02` → `/simplify` → `03` → `/qa` → `/ship` |
| 버그 수정 (S) | `/investigate` → `/{fw}-04-fix` → `/ship` |
| 병렬 마이그레이션 | `/batch` |
| 코드 정리 | `/simplify` |
